...
wb-fiscal-watch:  ## WBF.S4: has the fiscal dashboard changed? (exit 1 = yes)
	$(PY) -m ingestion.worldbank.fiscal_watch
...
kalimati-export:  ## Publish the WHOLE Kalimati dataset as public CSV downloads
	$(PY) scripts/kalimati_export.py
...
ecn-load:  ## ECN.S3: load election party votes + seats for both HoR elections
	$(PY) -m ingestion.election.ecn_pipeline
