"""Reusable loader for clean Shape-A NPHC 2021 household tables.

Each source-specific entrypoint supplies an explicit :class:`ShapeAConfig`.
This module owns the shared, verified mechanics: raw-first archive, strict
schema and aggregate reconciliation, curated numeric geography mapping,
quality gate, batched change-only writes, and ingestion lineage.
"""
...
class ShapeAConfig:
    ...
    expected_national_total: Decimal
    ...
    with source_csv.open(encoding="utf-8-sig", newline="") as fh:
        ...
        if tuple(reader.fieldnames or ()) != expected_columns:
            return (
                ...
                [
                    ...
                    f"{list(expected_columns)}, got {reader.fieldnames or []}"
                ],
                level_counts,
                institutional_rows,
            )
        ...
        for line_number, row in enumerate(reader, 2):
            ...
            try:
                ...
                categories = {
                    column: Decimal(row[column]) for column in category_columns
                }
...
def run_shape_a(config: ShapeAConfig) -> int:
    ...
    if not config.source_csv.exists():
        print(f"FAILURE: {config.source_csv} not found (the NSO census download).")
        return 1
    ...
    stored = lake.store(
        ...
        config.source_csv.read_bytes(),
        f"file://{config.source_csv}",
        content_type="text/csv",
        payload_filename=config.source_csv.name,
    )
    raw_refs = [stored.payload_path]

    maps = load_geography_maps()
    values, failures, level_counts, institutional_rows = parse_source(
        config.source_csv,
        maps,
    ...
    if national != config.expected_national_total:
        failures.append(
            f"national rowtotal expected {config.expected_national_total}, got {national}"
        )
    for level in ("province", "district", "local_unit"):
        ...
        if subtotal != config.expected_national_total:
            failures.append(
                f"{level} headline rows sum to {subtotal}, "
                f"expected {config.expected_national_total}"
            )
    if failures:
    ...
    try:
        ...
        conn.close()
        print(
            f"FAILURE: {config.summary_name} census ingestion failed and was logged. "
            f"Reason: {exc}"
        )
        return 1

    conn.close()
    print(f"{config.summary_name} census ingestion summary:")
    print(f"  geographies         : {sum(level_counts.values())} {dict(level_counts)}")
    print(
        f"  values parsed       : {len(values):,} "
        f"(headline + {len(config.category_columns)} source-code breakdowns)"
    )
